...
"""
alternatively you can create new token without the 'sub' field, but ensure you're not missing any fields
in payload var.

exp = int(time.time()) +3600
payload = {"exp": exp, "type": "access"}
token = jwt.encode(payload, key=security.SECRET_KEY, algorithm=security.ALGOR)
"""
...
async def test_get_user_not_found():
    user = await security.get_user("test@example.ggg")
    assert user is None
...
# if test raises the error it means it pass
@pytest.mark.anyio
