...
async def create_post(body: str, async_client: AsyncClient, logged_in_token: str) -> dict:
    response = await async_client.post(
        "/post",
        json={"body": body},
        # token send in headers as a extra bit of information.
...
# mocker to create a token what already is expired.
@pytest.mark.anyio
async def test_create_post_expired_token(
    async_client: AsyncClient, confirmed_user: dict, mocker
):
    # to mock, pass the full path to the function, then returned value.
...
# now the test will run twice with different parameters
